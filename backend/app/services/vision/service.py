...
class VisionService:
    ...
    def analyze_page(
        ...
        return run_stage(
            ...
            lambda: self._analyze(layout, page_width),
            provider="heuristic",
        )

    def _analyze(self, layout: LayoutPageResult, page_width: float) -> VisionPageResult:
        ...
        for region_id, region in region_ids.items():
            ...
            hierarchy = self._hierarchy_level(region.element_type)
            alignment = self._estimate_alignment(region.bbox, page_width)
        ...
        self._link_captions(enrichments)
        self._link_headers_footers(enrichments, page_width)
        sections = self._build_sections(enrichments)
        column_count = self._detect_columns(regions, page_width)

        for e in enrichments:
            section = next((s for s in sections if e.region_id in s.block_ids), None)
            if section:
                e.vision.section_id = section.id
                e.vision.column_index = self._column_index(e.region.bbox, page_width, column_count)
    ...
    @staticmethod
    def _hierarchy_level(element_type: LayoutElementType) -> int:
    ...
    @staticmethod
    def _estimate_font_size(region: LayoutRegion) -> float:
        _, _, _, h = region.bbox
    ...
    @staticmethod
    def _link_captions(enrichments: list[VisionRegionEnrichment]) -> None:
        figures = [
            e
            for e in enrichments
    ...
    @staticmethod
    def _build_sections(enrichments: list[VisionRegionEnrichment]) -> list[Section]:
        sections: list[Section] = []
        current: Section | None = None
        order = 0
        for e in sorted(enrichments, key=lambda x: (x.region.bbox[1], x.region.bbox[0])):
            if e.region.element_type in {
    ...
    @staticmethod
    def _detect_columns(regions: list[LayoutRegion], page_width: float) -> int:
        if not regions:
            return 1
